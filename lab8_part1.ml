...
module MakeInterval (Endpoint : ORDERED_TYPE) = 
  struct
    ...
    let contains (intvl : interval) (x : Endpoint.t) : bool =
      match intvl with
        | Empty -> false 
        | Interval (a, b) -> Endpoint.compare x b <= 0 && Endpoint.compare x a >= 0 

    (* intersect intvl1 intvl2 -- Returns the intersection of `intvl1`
       and `intvl2` *)
    let intersect (intvl1 : interval) (intvl2 : interval) : interval =
      let max a b = 
        let v = Endpoint.compare a b in if v >= 0 then b else a in
      let min a b = 
        let v = Endpoint.compare a b in if v <= 0 then b else a in
      match intvl1, intvl2 with
      | Interval (a,b), Interval (c,d) -> 
        if contains intvl1 c || contains intvl1 d then create (max a c) (min b d)
        else Empty
      | _, _ -> Empty
...
module IntInterval = MakeInterval (
  struct 
    type t = int
    let compare (x : t) (y : t) = if x < y then (-1) 
...
let intvl1 = IntInterval.create 1 12 ;;
let intvl2 = IntInterval.create 2 15;;
...
module MakeBestInterval (Endpoint : ORDERED_TYPE) : (INTERVAL with type endpoint 
= Endpoint.t) =
  struct
    type endpoint = Endpoint.t
    type interval =
